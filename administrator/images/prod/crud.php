...
class loader{
	...
	public function loader_Vision($db){
		...
		while($rows=$res->fetch(PDO::FETCH_ASSOC)){
	       $strOut.='<div style="float:left;margin-left:1px" >';
			$strOut.='<p style="margin-top:10px;font-size:15px;"><b style="color:#9dcf56;font-size:18px">'.$rows['category'].'</b>&nbsp&nbsp'.$rows['vmcEng'].'</p>';	           	
			$strOut.='   </div>';
			 echo $strOut;
		}
	}
	...
	public function loader_marcelo($db){
		...
		$res->execute();
				while($rows=$res->fetch(PDO::FETCH_ASSOC)){
				$strOut.='          <ul style=" font-family:Arial, Helvetica, sans-serif;font-size:20px">';
	...
	public function BoardofTrustees($db){
		...
		echo $strBot;*/
						...
						while($row=$res->fetch(PDO::FETCH_ASSOC)){
						$strBot.='<tr><td width="110px"><img src="administrator/images/mng/BOT/'.$row['boardPics'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row['BoardName'].'</strong><br/>'.$row['boardCompany'].'</td></tr>';
						}
						$strBot.='</table>';
						echo $strBot;
						...
						$strBot1.='<tr><td width="110px"><img src="administrator/images/mng/BOT/'.$row1['boardPics'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row1['BoardName'].'</strong><br/>'.$row1['boardCompany'].'</td></tr>';
						}
						$strBot1.='</table>';
						echo $strBot1;
						...
						$strBot2.='<tr><td width="110px"><img src="administrator/images/mng/BOT/'.$row2['boardPics'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row2['BoardName'].'</strong><br/>'.$row2['boardCompany'].'</td></tr>';
						}
						$strBot2.='</table>';
						echo $strBot2;
	...
	public function steeringCommitee($db){
		...
		echo $strSc;*/
					...
					while($row1=$res1->fetch(PDO::FETCH_ASSOC)){
					$strSc1.='<tr><td width="110px"><img src="administrator/images/mng/SC/'.($row1['img']).'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.($row1['steerName']).'</strong><br/>'.$row1['steerPosition'].'</td></tr>';
					}
					$strSc1.='</table>';
					echo $strSc1;
					...
					while($row=$res->fetch(PDO::FETCH_ASSOC)){
					$strSc.='<tr><td width="110px"><img src="administrator/images/mng/SC/'.($row['img']).'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row['steerName'].'</strong><br/>'.$row['steerPosition'].'</td></tr>';
					}
					$strSc.='</table>';
					echo $strSc;
	}
		public function clusterManager($db){
					...
					while($row=$res->fetch(PDO::FETCH_ASSOC)){
					$strCm.='<tr><td width="110px"><img src="administrator/images/mng/CM/'.$row['cm_img'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row['cm_name'].'</strong><br/>'.$row['cluster'].'</td></tr>';
					}
					$strCm.='</table>';
					echo $strCm;
					...
					while($row2=$res2->fetch(PDO::FETCH_ASSOC)){
					$strCm2.='<tr><td width="110px"><img src="administrator/images/mng/CM/'.$row2['cm_img'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row2['cm_name'].'</strong><br/>'.$row2['cluster'].'</td></tr>';
					}
					$strCm2.='</table>';
					echo $strCm2;
	...
	public function FieldManager($db){
					...
					while($row=$res->fetch(PDO::FETCH_ASSOC)){
					$strFm.='<tr><td width="110px"><img src="administrator/images/mng/FM/'.$row['fm_img'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.($row['fm_name']).'</strong><br/>'.$row['fo'].'</td></tr>';
					}
					$strFm.='</table>';
					echo $strFm;
					...
					while($row2=$res2->fetch(PDO::FETCH_ASSOC)){
					$strFm2.='<tr><td width="110px"><img src="administrator/images/mng/FM/'.$row2['fm_img'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row2['fm_name'].'</strong><br/>'.$row2['fo'].'</td></tr>';
					}
					$strFm2.='</table>';
					echo $strFm2;
	...
	public function SbookKeeper($db){
					...
					while($row=$res->fetch(PDO::FETCH_ASSOC)){
					$strSbk.='<tr><td width="110px"><img src="administrator/images/mng/SBK/'.$row['sbk_img'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row['sbk_name'].'</strong><br/>'.$row['cluster'].'</td></tr>';
					}
					$strSbk.='</table>';
					echo $strSbk;
					...
					while($row2=$res2->fetch(PDO::FETCH_ASSOC)){
					$strSbk2.='<tr><td width="110px"><img src="administrator/images/mng/SBK/'.$row2['sbk_img'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row2['sbk_name'].'</strong><br/>'.$row2['cluster'].'</td></tr>';
					}
					$strSbk2.='</table>';
					echo $strSbk2;
	...
	public function OMCM($db){
						...
						while($row=$res->fetch(PDO::FETCH_ASSOC)){
						$stromcm.='<tr><td width="110px"><img src="administrator/images/mng/OMCM/'.$row['omcm_img'].'" style="margin:5px 5px 5px 5px ; width:100px;height:100px"/></td><td style="padding-left:3px" ><strong>'.$row['omcm_name'].'</strong><br/>'.$row['pos'].'</td></tr>';
						}
						$stromcm.='</table>';
						echo $stromcm;
	...
	public function opan($db){
			$sqlOpan="SELECT * FROM tblpartners GROUP BY partnersNumber";
						$res=$db->prepare($sqlOpan);
						$res->execute();
						while($row=$res->fetch(PDO::FETCH_ASSOC)){  
					//	$a = 'select * , count(fo_id) from tblfo';
						...
						//$strfoL.='</table>';
							$sqlfo1="SELECT * FROM tblpartners where partnersNumber = ? ";
							$res1=$db->prepare($sqlfo1);
							$res1->execute(array($row['partnersNumber']));
							//$strfoL.='<br/><br/><table  width="400px"  style="font-family:Arial, Helvetica, sans-serif;font-size:14px;float:right;margin-left:25px ">';
							while($row2=$res1->fetch(PDO::FETCH_ASSOC)){
							$strOpan.='<table  style="font-family:Arial, Helvetica, sans-serif;font-size:14px;margin-left:25px ">';
							$strOpan.='<tr><td style="padding-left:3px" >'.$row2['partnersName'].'</td></tr>';
							}
							$strOpan.='</table>';
						}
						echo $strOpan;
	...
	public function loader_job($db){
		...
		$strOut.='<img src="administrator/images/jobs/job.jpg" width="920px" height="200px" /><br/><br/><br/>';
		$strOut.='<p style="font-size:14px">KDCI welcomes to its staff individuals who are willing to serve the poor by giving them opportunities for microentrepreneurship and helping provide for their other socioeconomic needs.</p>';
		while($rows=$res->fetch(PDO::FETCH_ASSOC)){
					$strOut.='<br/><br/><p style="font-family:Arial, Helvetica, sans-serif;font-size:25px;color:#9dcf56;" >'.$rows['jobTitle'].'</p>';
				$strOut.=stripslashes($rows['jobDesc']);//595	
...
/********************Product*********************************/
		...
		public function loader_product($db,$pId){
	
		$strOut="";
		$perPage=5;
		$strprod="Select * from tblproduct";
		$res=$db->prepare($strprod);
		$res->execute();
		$rCount=$res->rowCount();
		$numPage=$rCount/$perPage;
			...
			$strprod="SELECT * FROM tblproduct WHERE prodDescId <> ? limit {$start},{$perPage}";
			$res2=$db->prepare($strprod);
			$res2->execute(array($pId));
			}
	        ...
	        $strOut.='        <a href="products.php?pId='.$rows['prodDescId'].'"><img src="administrator/images/prod/'.$rows['prodPics'].'" alt="Image 12" class="image_frame_300" /></a>';
	        ...
	        $strOut.='           <a href="products.php?pId='.$rows['prodDescId'].'" style="color:#9dcf56"><p align="right">More...</p></a>';
	        $strOut.='        </div>';
			$strOut.='	</div>';
	        $strOut.='    <div ></div>';
	        $strOut.='</div>';
	...
	/****************end Product*************/
/********************News*********************************/
	public function loader_News($db,$nId){
	
		$strOut="";
		$perPage=5;
		$strNewsC="Select * from tblnews ORDER BY newsId DESC";
		...
		$numPage=$rCount/$perPage;
			...
			if($nId==null){
			$strNews="SELECT * FROM tblnews ORDER BY newsId desc limit {$start},{$perPage}  ";
			$res2=$db->prepare($strNews);
			$res2->execute();
			}else{
			$strNews="SELECT * FROM tblnews WHERE newsId <> ? ORDER BY newsId desc limit {$start},{$perPage}  ";
			$res2=$db->prepare($strNews);
			$res2->execute(array($nId));
			}
			...
			$strOut.='<div class="post_box">';
	        $strOut.='    <div class="col_b float_l" style="margin-top:10px;margin-left:10px;width:890px">';
	        $strOut.='        <a href="newsandfetured.php?nId='.$rows['newsId'].'"><img src="administrator/images/news/'.$rows['news_pics'].'" alt="Image 12" class="image_frame_300" /></a>';
	        $strOut.='        <div class="post_text" style="width:560px">';
	        ...
	        $strOut.='           <a href="newsandfetured.php?nId='.$rows['newsId'].'" style="color:#9dcf56"><p align="right">More...</p></a>';
	        $strOut.='        </div>';
			$strOut.='	</div>';
	        $strOut.='    <div ></div>';
	        $strOut.='</div>';
	...
	public function view_news($db,$nId){
		...
		if($rCount!=nulll){
			if($rows=$res->fetch(PDO::FETCH_ASSOC)){
				...
				$strOut.='<p style="font-family:Arial, Helvetica, sans-serif;font-size:20px;font-weight:bold" >'.$rows['news_title'].'</p>';
				
				$strOut.=stripslashes($rows['news_desc']);//595
			}
			$strOut.='<p style="font-family:Arial, Helvetica, sans-serif;font-size:25px;color:black;margin-top:50px">Other News and Events</p>';
			echo $strOut;
		}
	}
	/****************end Product*************/
	...
	public function loader_story($db,$nId){
		
		$strOut="";
		$perPage=5;
		$strstory1="Select * from tblstory";
		$res=$db->prepare($strstory1);
		$res->execute();
		$rCount=$res->rowCount();
		$numPage=$rCount/$perPage;
			if (isset($_GET['start'])){
			...
			if($nId==0){
			$strstory="SELECT * FROM tblstory limit {$start},{$perPage}";
			$res2=$db->prepare($strstory);
			$res2->execute();
			}else{
			$strstory="SELECT * FROM tblstory WHERE story_id <> ? limit {$start},{$perPage}";
			$res2=$db->prepare($strstory);
			$res2->execute(array($nId));
			}
			...
			$strOut.='<div class="post_box">';
	        $strOut.='    <div class="col_b float_l" style="margin-top:10px;margin-left:10px;width:890px">';
	        $strOut.='         <a href="partners.php?nId='.$rows['story_id'].'"><img src="administrator/images/story/'.$rows['story_img'].'"  alt="Image 12" class="image_frame_160" " style="margin-right:10px" /></a>';
	        $strOut.='        <div class="post_text" style="width:560px">';
	        $strOut.='        <p style="font-family:Arial, Helvetica, sans-serif;font-size:17px;color:black">'.$rows['story_title'].'</p>';
	        $strOut.=substr(nl2br(stripslashes($rows['story_desc'])),0,273);
	      
	        $strOut.='           <a href="partners.php?nId='.$rows['story_id'].'" style="color:#9dcf56"><p align="right">More...</p></a>';
	        $strOut.='        </div>';
			$strOut.='	</div>';
	        $strOut.='    <div ></div>';
	        $strOut.='</div>';
	...
	public function view_story($db,$nId){
		...
		if($rCount!=null){
			if($rows=$res->fetch(PDO::FETCH_ASSOC)){
				$strOut.='<img src="administrator/images/story/'.$rows['story_img'].'"  alt="Image 12" class="image_frame_160" " style="margin-right:10px" >';
				$strOut.='<p style="font-family:Arial, Helvetica, sans-serif;font-size:17px;font-weight:bold" ><a href="#">'.$rows['story_title'].'</a></p>';
				
				$strOut.=stripslashes($rows['story_desc']);//595
				  $strOut.='        	<p style="font-family:Arial, Helvetica, sans-serif;font-size:17px;color:black">'.$rows['story_date'].'</p>';
			}
			$strOut.='<p style="font-family:Arial, Helvetica, sans-serif;font-size:25px;color:black;margin-top:50px">Other Stories and Features</p>';
			echo $strOut;
		}
	}
	...
	public function news_home($db){
		...
		while($rows=$res->fetch(PDO::FETCH_ASSOC)){
				...
				$strOut.='<p style= font-family:Arial, Helvetica, sans-serif; font-size:16px; font-weight:100><a href="newsandfetured.php?nId='.$rows['newsId'].'"><b>'.$rows['news_title'].'</b></a></p>';
				$strOut.='<p>'.substr(nl2br(stripslashes($rows['news_desc'])),0,197).'</p><a href="" align="right"></a>';
	...
	public function social($db){
		...
		while($rows=$res->fetch(PDO::FETCH_ASSOC)){
			$strOut1.='<a href="'.$rows['social_url'].'" target="_blank"><img src="administrator/images/social/'.$rows['social_pics'].'"></a>&nbsp';
		}
		$strOut1.='</p> ';
		echo $strOut1; 
	}
