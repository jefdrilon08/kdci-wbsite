...
class loader{
	...
	public function loader_job($db){
		...
		$strOut.='<p>KDCI welcomes to its staff individuals who are willing to serve the poor by giving them opportunities for microentrepreneurship and helping provide for their other socioeconomic needs.</p>';
		while($rows=$res->fetch(PDO::FETCH_ASSOC)){
...
/********************Product*********************************/
	public function loader_product($db){
		...
		$res2->execute();	
	        ...
	        $strOut.='        	<p style="font-family:Arial, Helvetica, sans-serif;font-size:17px;color:#9dcf56">'.$rows['prodcategId'].'</p>';
...
/********************News*********************************/
	...
	public function loader_story($db){
		...
		$res2->execute();	
			...
			$strOut.='<div class="post_box">';
	        $strOut.='    <div class="col_b float_l" style="margin-top:10px;margin-left:10px;width:890px">';
	        $strOut.='        <img src="administrator/images/story/'.$rows['story_img'].'" alt="Image 12" class="image_frame_160" />';
	        $strOut.='        <div class="post_text" style="width:560px">';
