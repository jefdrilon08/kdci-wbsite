...
/********************Product*********************************/
		...
		$numPage=$rCount/$perPage;
    		...
    		$words2 = explode(" ",stripslashes($rows['prodDesc']));
	        $strOut.=implode(" ",array_splice($words2,0,100));
...
/********************News*********************************/
	public function loader_News($db,$nId){
		...
		$numPage=$rCount/$perPage;
			...
			$strNews="SELECT * FROM tblnews WHERE newsId <> ? limit {$start},{$perPage}";
			$res2=$db->prepare($strNews);
	        ...
	        $words2 = explode(" ",stripslashes($rows['news_desc']));
	        $strOut.=implode(" ",array_splice($words2,0,100));
	...
	public function miniSlider($db){
		...
		$strOut.='<ul>';
		while($rows=$res->fetch(PDO::FETCH_ASSOC)){
			$strOut.='	<li style="margin-right:18px"><a href="" title="jef"><img src="administrator/images/prod/'.$rows['prodPics'].'" title="'.$rows['prodcategId'].'" alt="'.$rows['prodcategId'].'" class="image_frame_300" ><p align="center" style="font-style:italic;color:#9dcf56">'.$rows['prodcategId'].'</p></a></li>';	
		}
		$strOut.='</ul>';
	...
	public function loader_story($db,$nId){
		...
		$numPage=$rCount/$perPage;
	        ...
	        $strOut.='        <div class="post_text" style="width:695px">';
	        $strOut.='        	<p style="font-family:Arial, Helvetica, sans-serif;font-size:17px;color:black">'.$rows['story_title'].'</p>';
	         $words2 = explode(" ",stripslashes($rows['story_desc']));
	        $strOut.=implode(" ",array_splice($words2,0,93));
	...
	public function news_home($db){
		...
		while($rows=$res->fetch(PDO::FETCH_ASSOC)){
				...
				$words1 = explode(" ",stripslashes($rows['news_desc']));
				$strOut.='<p>'.implode(" ",array_splice($words1,0,10)).'</p><a href="" align="right"></a>';
